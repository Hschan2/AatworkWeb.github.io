export class Sun {
    ...
    draw(ctx, t) {
        ...
        ctx.fillStyle = '#FFF136';
        ctx.beginPath();
        // ctx.arc(this.x, this.y, this.radius, 0, 2 * Math.PI);
        // ctx.fill();
        let pos = this.pos[0];
        ctx.moveTo(pos.x + this.x, pos.y + this.y); // 업데이트된 좌표를 서로 선으로 연결
    ...
    updatePoints() {
        for(let i=1; i<this.total; i++) {
            ...
            this.pos[i] = {
                x: pos.x + this.ranInt(5), // x축 랜덤으로 지글, 최대 +5
                y: pos.y + this.ranInt(5), // y축 랜덤으로 지글, 최대 +5
            }
        }
    }
